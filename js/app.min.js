(() => {
    ...
    paralaxMouseInit(t) {
      t.forEach((t) => {
        const e = t.closest("[data-prlx-mouse-wrapper]"),
          ...
          a = t.dataset.prlxCy ? +t.dataset.prlxCy : 100,
        ...
        !(function e() {
          (i += ((l - i) * c) / 1e3),
            (r += ((d - r) * c) / 1e3),
            (t.style.cssText = `transform: translate3D(${(s * i) / (o / 10)}%,${
              (n * r) / (a / 10)
            }%,0) rotate(0.02deg);`),
  ...
  let u;
  function g() {
    (u = window.innerWidth < 1590 ? 15 : (window.innerWidth - 1560) / 2),
      d.length &&
        d.forEach((t) => {
  ...
  async function m(t) {
    try {
      const e = t.target.closest(".copy-ca-btn").textContent;
